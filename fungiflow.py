...
def get_args():
    ...
    try:
        ...
        parser.add_argument(
            ...
            "--array",
            action="store",
            help="an informative and unique string",
            type=str,
            required=True,
        )
        parser.add_argument(
            ...
            action="store",
            default=None,
            help="Illumina short forward reads path",
            type=str,
            required=False,
        )
        parser.add_argument(
            ...
            action="store",
            default=None,
            help="Illumina short reverse reads path",
            type=str,
            required=False,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            required=False,
            default=None,
            help="Path to installed databases",
            type=str,
        )
        parser.add_argument(
            "-s",
            ...
            action="store",
            required=False,
            help="Primary Singularity image for Fungiflow",
            type=str,
        )
        parser.add_argument(
            "-f",
            ...
            action="store_true",
            required=False,
            help="Add this argument if you would like to annotate the assembly. Not compatible with metagenomic mode.",
        )
        parser.add_argument(
            "-sf",
            "--singularity_funannotate",
            action="store",
            help="Singularity image for Funannotate",
            type=str,
            required=False,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "-sa",
            "--singularity_antismash",
            action="store",
            help="Singularity image for antiSMASH",
            type=str,
            required=False,
        )
        ...
        parser.add_argument(
            ...
            action="store",
            required=False,
            help="Path to alternative EggNOG database for EggNOG.",
            type=str,
        )
        parser.add_argument(
            "-its",
        ...
        parser.add_argument(
            ...
            action="store",
            required=False,
            help="Path to alternative ITS_refseq BLASTn database.",
            type=str,
        )
        parser.add_argument(
            "-k",
        ...
        parser.add_argument(
            ...
            action="store",
            required=False,
            help="Path to alternative Kraken2 standard database.",
            type=str,
        )
        ...
        parser.add_argument(
            ...
            action="store",
            default=None,
            help="Path to GeneMark-ES script.",
            type=str,
            required=False,
        )
        parser.add_argument(
        ...
        if args.singularity_funannotate and not args.funannotate:
            parser.error(
                "argument -sf/--singularity_funannotate: requires -f/--funannotate"
            )
        if args.singularity_antismash and not args.antismash:
            parser.error(
                "argument -sa/--singularity_antismash: requires -ant/--antismash"
            )
        ...
        logging.error("An exception occurred with argument parsing. Check your inputs.")
...
def main():
    ...
    logging.basicConfig(
        filename=os.path.join(
            input_args.directory, f"{current_datetime}_fungiflow.log"
        ),
        level=logging.INFO,
    ...
    input_args.directory_new = os.path.abspath(
        os.path.join(input_args.directory, input_args.array)
    )
    ...
    if input_args.print_workflow is True:
        ...
        if input.args.nanopore is None:
            (
                trimming
                + f"MinION reads were trimmed with porechop and length-filtered to \
                    {input_args.minimum_length} bp with bbduk.sh before being converted \
                        to FASTA format. Short reads were mapped to these reads and corrected \
                            using ropebwt2 and FMLRC."
            )
            assembly_proc = "Corrected long reads were assembled with Flye"
        else:
            if input_args.careful is True:
                parameters = " with the 'careful' parameter"
            else:
                parameters = ""
        ...
        if input_args.eggnog is True:
            if input_args.interproscan is True:
                iprscan = " and InterProScan"
            annotation = (
                annotation
                + f"Functional annotation was performed using Eggnog{iprscan}."
            )
        post = "Assemblies were evaluated with Quast."
        if input_args.its is True:
            post = post + "ITS regions were extracted using ITSx."
        if input_args.antismash is True:
            post = post + "BGCs were predicted using antiSMASH."
        print(f"{trimming} {assembly_proc} {annotation} {post}")
        logging.info(f"{trimming} {assembly_proc} {annotation} {post}")
