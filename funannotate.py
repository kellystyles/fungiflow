...
def funannotate_clean(input_args, filenames):
    ...
    cmd = [
        ...
        "-o",
        filenames.funannotate_clean_fasta,
        "--cpus",
        input_args.cpus,
    ]
    if len(filenames.funannotate) > 0:
        cmd = filenames.funannotate + cmd

    lib.execute(cmd, stdout, stderr)
    lib.file_exists_exit(
        filenames.funannotate_clean_fasta,
        "Assembly successfuly cleaned by Funannotate",
        "Funannotate clean failed... check the logs and your inputs",
    )
...
def funannotate_sort(input_args, filenames):
    ...
    lib.file_exists_exit(
        filenames.funannotate_sort_fasta,
        "Assembly successfuly sorted by Funannotate",
        "Funannotate sort failed... check the logs and your inputs",
    )
...
def funannotate_mask(input_args, filenames):
    ...
    cmd = [
        ...
        filenames.funannotate_sort_fasta,
        "-o",
        filenames.funannotate_mask_fasta,
        "--cpus",
        input_args.cpus,
    ]
    if len(filenames.funannotate) > 0:
    ...
    lib.file_exists_exit(
        filenames.funannotate_mask_fasta,
        "Assembly successfully masked by Funannotate",
        "Funannotate mask failed... check the logs and your inputs",
    )
...
def funannotate_predict(input_args, filenames, funannotate_path):
    ...
    cmd = [
        ...
        input_args.array,
        "--min_training_models",
        input_args.min_training_models,
    ]
    if len(filenames.funannotate) > 0:
        cmd = filenames.funannotate + cmd
    if input_args.genemark_path is not None:
        cmd = cmd + ["--GENEMARK_PATH", input_args.genemark_path]
        ...
        with open(stderr, "r") as f:
            ...
            if "error, file not found: data/training.fna" in lines[-2]:
                gme_cmd = [
                    ...
                    "--sequence",
                    os.path.join(
                        funannotate_path, "predict_misc", "genome.softmasked.fa"
                    ),
                    "--fungus",
                    "--min_contig",
                    "2000",
                    "--work_dir",
                    os.path.join(funannotate_path, "predict_misc"),
                ]
                if len(filenames.funannotate) > 0:
                    gme_cmd = filenames.funannotate + gme_cmd
                lib.print_n(
                    "GeneMark ES could not predict genes from the assembly as it \
                        is too fragmented. Will try to optimize the parameters for low-coverage assembly..."
                )
                lib.execute(gme_cmd, stdout, stderr)
                cmd = cmd + [
                    "--genemark_gtf",
                    os.path.join(funannotate_path, "predict_misc", "genemark.gtf"),
                ]
                lib.execute(cmd, stdout, stderr)
            lib.file_exists(
                filenames.funannotate_gbk,
                ...
                "Funannotate predict failed... check the logs and your inputs\n \
                    You could try using the `--careful` input parameter to prepare \
                        a better assembly. You may need to remove the existing `assembly` directory first.",
            )
...
def eggnog_annotate(input_args, filenames, eggnog_path):
    ...
    cmd = [
        ...
        "diamond",
        "-i",
        filenames.funannotate_prots,
        "--data_dir",
        input_args.eggnog_db,
        "-o",
        output_prefix,
        "--tax_scope",
        "4751,33154,2759,1",
    ...
    if int(input_args.mem) > 45000:
        cmd = cmd + ["--dbmem"]
    # If a hits file from a previous run is present, resume the run
    if lib.file_exists(
        os.path.join(eggnog_path, f"{input_args.array}.emapper.hits"),
        "Existing EggNog hits file found. Resuming previous run...",
        "",
    ):
        cmd = cmd + ["--resume"]
    ...
    lib.execute(cmd, stdout, stderr)
    lib.file_exists_exit(
        filenames.eggnog_annotations,
        "CDS successfully annotated with eggnog mapper",
        "eggnog mapper failed... check the logs and your inputs",
    )
...
def funannotate_annotate(input_args, filenames, funannotate_path):
    ...
    cmd = [
        ...
        "annotate",
        "-i",
        os.path.join(funannotate_path, "predict_results"),
        "-o",
        funannotate_path,
        "-s",
        input_args.array,
    ...
    if lib.file_exists(filenames.eggnog_annotations, "", "") is True:
        ...
        try:
            os.remove(
                os.path.join(
                    funannotate_path, "annotate_misc", "eggnog.emapper.annotations"
                )
            )
...
def main(input_args, filenames):
    ...
    if (
        lib.is_conda_environment_available("fungiflow-funannotate")
        and getattr(input_args, "singularity_funannotate", None) is None
    ):
        lib.print_n("Activating the Funannotate conda environment")
        lib.activate_conda_env("fungiflow-funannotate")
    elif (
        lib.is_conda_environment_available("fungiflow-funannotate") is False
        and getattr(input_args, "singularity_funannotate", None) is not None
    ):
        lib.print_n(f"Using {input_args.singularity_funannotate} to run Funannotate")
    elif (
        lib.is_conda_environment_available("fungiflow-funannotate")
        and getattr(input_args, "singularity_funannotate", None) is not None
    ):
    ...
    # Defining filenames
    filenames.funannotate_clean_fasta = os.path.join(
        funannotate_path, f"{input_args.array}_cleaned.fasta"
    )
    filenames.funannotate_sort_fasta = os.path.join(
        funannotate_path, f"{input_args.array}_sorted.fasta"
    )
    filenames.funannotate_mask_fasta = os.path.join(
        funannotate_path, f"{input_args.array}_masked.fasta"
    )
    filenames.funannotate_gbk = os.path.join(
        funannotate_path, "predict_results", f"{input_args.array}.gbk"
    )
    filenames.funannotate_gff = os.path.join(
        funannotate_path, "predict_results", f"{input_args.array}.gff3"
    )
    filenames.funannotate_prots = os.path.join(
        funannotate_path, "predict_results", f"{input_args.array}.proteins.fa"
    )
    filenames.eggnog_annotations = os.path.join(
        eggnog_path, f"{input_args.array}.emapper.annotations"
    )
    filenames.ipr_annotations = os.path.join(iprscan_path, f"{input_args.array}.xml")
    filenames.funannotate_func_gbk = os.path.join(
        funannotate_path, "annotate_results", f"{input_args.array}.gbk"
    )
    ...
    if (
        lib.file_exists(
            filenames.funannotate_clean_fasta,
            "Assembly already cleaned by Funannotate! Skipping...",
            "",
        )
        is False
    ):
    ...
    if (
        lib.file_exists(
            filenames.funannotate_sort_fasta,
            "Assembly already sorted by Funannotate! Skipping...",
            "",
        )
        is False
    ):
    ...
    if (
        lib.file_exists(
            filenames.funannotate_mask_fasta,
            "Assembly already masked by Funannotate! Skipping...",
            "",
        )
        is False
    ):
    ...
    if (
        lib.file_exists(
            filenames.funannotate_gbk,
            "Assembly genes already predicted with Funannotate! Skipping...",
            "",
        )
        is False
    ):
    ...
    if input_args.eggnog is True:
        if (
            lib.file_exists(filenames.funannotate_gbk, "", "") is True
            and lib.file_exists(
                filenames.eggnog_annotations,
                "Functional annotation already completed with eggnog",
                "",
            )
            is False
        ):
    ...
    if (
        lib.file_exists(
            filenames.funannotate_func_gbk,
            "Functional annotation with Funannotate already performed! Skipping...",
            "",
        )
        is False
    ):
    ...
    lib.print_h(
        f"funannotate module completed in {datetime.datetime.now() - funannotate_start_time}"
    )
