        ...
        for i in range(len(json_object["records"])):
            ...
            for j in range(len(json_object["records"][i]["features"])):
                ...
                if "cand_cluster" in record["type"]:
                    ...
                    a["BGC_location"] = (
                        str(record["location"]).lstrip("[<").rstrip(">]")
                    )
                    ...
                    if "antismash.modules.clusterblast" in kc_record:
                        for c in kc_record["antismash.modules.clusterblast"]["general"][
                            "results"
                        ]:
                            if len(c["ranking"]) != 0:
...
def flatten_antismash(antismash_json, bgc_results, final_df):
    ...
    try:
        ...
        final_df["kc_proportion"] = [
            round(
                len(bgc_df[bgc_df["kc_mibig_acc"] != ""])
                / int(bgc_df["kc_mibig_acc"].size),
                2,
            )
        ]
        ...
        pass
    try:
        final_df["contig_edge_proportion"] = [
            round(
                int(bgc_df["partial"].value_counts().loc["True"])
                / int(bgc_df["partial"].size),
                2,
            )
        ]
    except KeyError:
...
def main():
    ...
    directories[:] = [
        ...
        if os.path.isdir(os.path.join(main_dir, d, "assembly")) is True
    ]
    n = 0
    ...
    for dir in directories:
        ...
        try:
            ...
            pass
        try:
            funannotate_json = os.path.join(
                main_dir, dir, "funannotate", "predict_results", f"{dir}.stats.json"
            )
        ...
        try:
            if lib.file_exists(
                os.path.join(main_dir, dir, "ITSx", f"{dir}_its_blastn.out"), "", ""
            ):
                itsx_output = os.path.join(
                    main_dir, dir, "ITSx", f"{dir}_its_blastn.out"
                )
