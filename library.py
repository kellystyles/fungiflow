...
class Files:
    ...
    def __init__(self, input_args):
        ...
        if input_args.illumina_f is not None and input_args.illumina_r is not None:
            self.shortf = os.path.abspath(
                os.path.join(input_args.directory, input_args.illumina_f)
            )
            self.shortr = os.path.abspath(
                os.path.join(input_args.directory, input_args.illumina_r)
            )
        else:
            self.shortf = None
            self.shortr = None
        ...
        if input_args.singularity_funannotate is not None:
            self.funannotate = [
                "singularity",
                "exec",
                input_args.singularity_funannotate,
            ]
        else:
            self.funannotate = ""
...
def execute(command, stdout, stderr):
    ...
    try:
        ...
        with open(stdout, "wt", encoding="utf-8") as out, open(
            stderr, "wt", encoding="utf-8"
        ) as err:
        ...
        logging.error(f"Exit code: {error.returncode}")
        logging.error(f"Output: {error.output}")
        logging.error(f"Error: {error.stderr}")
    ...
    try:
        with open(stdout, "wt", encoding="utf-8") as out, open(
            ...
            if len(command_list) == 1:
                ...
                logging.info(command_list[0])
                subprocess.run(
                    command_list[0], stdout=out, stderr=err, shell=True, check=False
                )
            else:
...
def download_file(url, save_path):
    ...
    with open(save_path, mode) as file, tqdm.tqdm(
        total=total_size,
    ...
    if downloaded_size != total_size:
        logging.error("Downloaded file size doesn't match the expected size.")
...
def check_databases(input_args, filenames_class_obj):
    ...
    try:
        if input_args.database_path is not None:
            if input_args.its is True:
                its_db = os.path.abspath(
                    os.path.join(input_args.database_path, "ncbi-its")
                )
            if input_args.kraken2 is True:
                kraken2_db = os.path.abspath(
                    os.path.join(input_args.database_path, "kraken2")
                )
            if input_args.eggnog is True:
                eggnog_db = os.path.abspath(
                    os.path.join(input_args.database_path, "eggnog")
                )
        else:
            try:
        ...
        logging.error("Please supply a value for `--database_path`")
    except TypeError:
        print("Please supply a value for `--database_path`")
        logging.error("Please supply a value for `--database_path`")
    ...
    try:
        ...
        if (
            file_exists_list(
                [ncbi_its, taxdb],
                "ITS DB is present",
                "ITS DB is not present. \
            Please supply a path via `--its_db` or run install.py again.",
            )
            is False
        ):
            c.append(its_db)
    ...
    try:
        ...
        if (
            file_exists_list(
                [hash, opts, taxo],
                "kraken2 DB is present",
                "kraken2 DB is not present. \
            Please supply a path via `--kraken2_db` or run install.py again.",
            )
            is False
        ):
            c.append(kraken2_db)
    ...
    try:
        ...
        if (
            file_exists_list(
                [dmnd, taxa, main, prots, fungi_dmnd],
                "eggnog DB is present",
                "eggnog DB is not present. \
            Please supply a path via `--eggnog_db` or run install.py again.",
            )
            is False
        ):
            c.append(eggnog_db)
    ...
    if len(c) > 0:
        ...
        logging.error(
            "Please check your `--database_path` or other supplied database paths."
        )
        logging.error("Try running `install.py` again.")
        sys.exit(1)
...
def print_h(text):
    ...
    """
    print(
        f"{CYAN}{BOLD}[{datetime.now().strftime('%b %d %I:%M %p')}]{END}",
        f"{CYAN} {text} {END}",
    )
    logging.info(f"[{datetime.now().strftime('%b %d %I:%M %p')}] {text}")
...
def print_n(text):
    ...
    """
    print(
        f"{WHITE}[{datetime.now().strftime('%b %d %I:%M %p')}]{END}",
        f"{WHITE} {text} {END}",
    )
    logging.info(f"[{datetime.now().strftime('%b %d %I:%M %p')}] {text}")
...
def print_e(text):
    ...
    """
    print(
        f"{RED}{BOLD}[{datetime.now().strftime('%b %d %I:%M %p')}]{END}",
        f"{RED} {text} {END}",
    )
    logging.error(f"[{datetime.now().strftime('%b %d %I:%M %p')}] {text}")
