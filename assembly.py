...
def fastq_2_fasta(input_args,filenames,trimmed_path):
    ...
    cmd1 = ["singularity","exec","-B","/nfs:/nfs",input_args.singularity,"reformat.sh",f"in={filenames.nanopore_trimmed}",f"out={filenames.nanopore_fasta}","ignorebadquality=True"] 
    try:
        lib.execute(cmd1,stdout,stderr)
    except subprocess.CalledProcessError as e:
        print(e.returncode)
...
def length_filter_fasta(input_args,filenames,trimmed_path):
    ...
    cmd1 = ["singularity","exec","-B","/nfs:/nfs",input_args.singularity,"bbduk.sh",f"in={filenames.nanopore_fasta}",f"out={filenames.nanopore_length_filtered}",f"minlen={input_args.minimum_length}"]
    try:
        lib.execute(cmd1,stdout,stderr)
    except subprocess.CalledProcessError as e:
        print(e.returncode)
