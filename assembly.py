...
def minimap2(input_args, filenames, assembly_path, read_length):
    ...
    if read_length == "long":
        ...
        cmd = ["minimap2", "-ax", "sr", filenames.medaka_consensus, filenames.trimmedf, filenames.trimmedr, "-o", filenames.medaka_consensus_bam]
    else:
...
def samtools_sort(input_args, filenames, assembly_path):
    ...
    cmd = ["samtools", "sort", "-@", input_args.cpus, "-o", filenames.medaka_sorted_bam, filenames.medaka_consensus_bam]
    if len(filenames.singularity) > 0: cmd = filenames.singularity + cmd
    try:
        lib.execute(cmd, stdout, stderr)
...
def samtools_index(input_args, filenames, assembly_path):
    ...
    cmd = ["samtools", "index", "-@", input_args.cpus, filenames.medaka_sorted_bam]
    if len(filenames.singularity) > 0: cmd = filenames.singularity + cmd
    try:
        lib.execute(cmd, stdout, stderr)
...
def pilon(input_args, filenames, assembly_path):
    ...
    cmd = ["pilon", f"-Xmx{int(int(input_args.mem)*0.8)}G", "--genome", filenames.medaka_consensus, "--bam", filenames.medaka_sorted_bam, "--outdir", assembly_path]
    if len(filenames.singularity) > 0: cmd = filenames.singularity + cmd
    try:
        lib.execute(cmd, stdout, stderr)
