...
def index(input_args,filenames,blobpath):
    ...
    try:
        ...
        lib.file_exists_exit(filenames.indexfile,"Samtools successfully indexed BAM file.", \
            "Samtools indexing failed.")
    except subprocess.CalledProcessError as e:
        print(e.returncode)
        print(e.output)   
...
def megablast(input_args,filenames,blobpath):
    ...
    try:
        lib.execute_shell(commands,stdout,stderr)
        lib.file_exists_exit(filenames.megablast_out,"MegaBLAST successfully identified contigs.", \
            "MegaBLAST failed.")
    except subprocess.CalledProcessError as e:
        print(e.returncode)
        print(e.output)
...
def main(input_args,filenames):
    ...
    if lib.file_exists(filenames.bamfile, "Trimmed reads already mapped to contigs, skipping...","") is False:
        minimap2(input_args,filenames,blobpath)
    if lib.file_exists(filenames.indexfile, "BAM file already sorted, skipping...","") is False:
        index(input_args,filenames,blobpath)
    if lib.file_exists(filenames.megablast_out, "Contigs already assigned with megablast, skipping...","") is False:
        megablast(input_args,filenames,blobpath)
    if lib.file_exists(filenames.blob_png, "Blobtools already executed, skipping...","") is False:
