...
def itsx(input_args, filenames, itsx_path):
    ...
    cmd = [
        ...
        "-o",
        os.path.join(itsx_path, input_args.array),
        "-t",
        "F",
        "--cpu",
        input_args.cpus,
    ...
    try:
        ...
        lib.print_e(e.returncode)
        lib.print_e(e.output)
...
def parse_blast(results_path, array):
    ...
    if os.path.exists(results_path):
        ...
        try:
            ...
            dataframe["ID"] = array
            lib.print_n(dataframe)
            return dataframe
        except pd.errors.EmptyDataError:
            lib.print_n(f"No BLAST results obtained for {results_path}")
    else:
        lib.print_e(f"The BLAST for {results_path} was not completed successfully")
...
def quast(input_args, filenames, output_path):
    ...
    try:
        ...
        if lib.file_exists(filenames.funannotate_gff, "", "") is True:
            lib.print_n(
                f"Funannotate annotations GFF file exists, analysing {filenames.funannotate_gff} and {filenames.assembly_fasta} with quast"
            )
            cmd = [
                "quast.py",
                filenames.assembly_fasta,
            ...
            lib.print_n(
                f"Funannotate annotations GFF file does not exist, analysing {filenames.assembly_fasta} with quast"
            )
            cmd = [
                "quast.py",
                filenames.assembly_fasta,
    ...
    try:
        ...
        lib.file_exists_exit(
            filenames.quast_report,
            "Quast successfully analysed the assembly!",
            "Quast failed... check the logs and your inputs",
        )
    except subprocess.CalledProcessError as e:
        lib.print_e(e.returncode)
        lib.print_e(e.output)
...
def parse_quast(filenames, array):
    ...
    try:
        ...
        lib.print_e(
            f"The QUAST report {filenames.quast_report} was not parsed successfully"
        )
...
def antismash(input_args, filenames, antismash_out):
    ...
    cmd = [
        ...
        "--output-basename",
        input_args.array,
        filenames.antismash_assembly,
    ]
    if len(filenames.antismash) > 0:
        cmd = filenames.antismash + cmd
    try:
        lib.print_n(
            f"Predicting gene clusters from {filenames.antismash_assembly} with antiSMASH"
        )
        lib.execute(cmd, stdout, stderr)
        lib.file_exists(
            filenames.antismash_json,
            "antiSMASH successfully analysed the assembly!",
            "antiSMASH failed... check the logs and your inputs",
        )
    except subprocess.CalledProcessError as e:
        lib.print_e(e.returncode)
        lib.print_e(e.output)
    ...
    try:
        with open(json_file, "r") as f:
            try:
                ...
                lib.print_e(f"JSON file {json_file} does not exist")
                return None
    except:
        lib.print_e(f"error occurred while parsing {json_file}:", sys.exc_info()[1])
        return None
...
def plot_violin(dataframe, input_args, results_path):
    ...
    ax = sns.violinplot(
        data=dataframe,
        x=dataframe["BGC_length"],
        y=dataframe["BGC_type"],
        group_by=dataframe["BGC_type"],
        palette="tab10",
        dodge=False,
    )
    ax.get_figure().savefig(
        os.path.join(results_path, f"{input_args.array}_bgctype_vs_bgclength.svg")
    )
    plt.clf()
...
def plot_strip(dataframe, input_args, results_path):
    ...
    for line in range(0, dataframe.shape[0]):
        if flag == True:
            ...
            flag = True
        ax.text(
            x=dataframe.BGC_length[line],
            y=offset,
            s=str(dataframe.kc_desc[line].split("/")[0]),
    ...
    ax.get_figure().savefig(
        os.path.join(results_path, f"{input_args.array}_contigedge_vs_bgclength_kc.svg")
    )
    plt.clf()
...
def parse_json(json_object, json_file):
    ...
    for i in range(len(json_object["records"])):
        ...
        if "antismash.detection.hmm_detection" in json_object["records"][i]["modules"]:
            while types_c < 1:
                types = json_object["records"][i]["modules"][
        ...
        for j in range(len(json_object["records"][i]["features"])):
            ...
            if "cand_cluster" in record["type"]:
                ...
                if "antismash.modules.clusterblast" in kc_record:
                    for kc in kc_record["antismash.modules.clusterblast"][
                        "knowncluster"
                    ]["results"]:
                        ...
                        if kc_total_hits > 0:
                            ...
                            a["kc_core_gene_hits"] = int(
                                kc["ranking"][0][1]["core_gene_hits"]
                            )
                            a["kc_blast_score"] = int(
                                kc["ranking"][0][1]["blast_score"]
                            )
                            a["kc_synteny_score"] = int(
                                kc["ranking"][0][1]["synteny_score"]
                            )
    ...
    try:
        ...
        for group in grouped:
            # print(type(group))
            single_locations = group[1][group[1]["kind"] == "single"][
                "BGC_location"
            ].tolist()
            neighbouring_locations = group[1][group[1]["kind"] == "neighbouring"][
                "BGC_location"
            ].tolist()
            for single in single_locations:
                ...
                for neighbouring in neighbouring_locations:
                    neighbouring_left, neighbouring_right = [
                        int(x) for x in neighbouring.split(":")
                    ]
                    if (
                        single_left >= neighbouring_left
                        and single_right <= neighbouring_right
                    ):
                        to_drop.extend(
                            group[1][group[1]["BGC_location"] == single].index
                        )
    ...
    if len(array) > 0:
        ...
        array["BGC_type"] = (
            array["BGC_type"].astype(str).str.replace("[\]\[]", "").str.replace("'", "")
        )
...
def parse_antismash(input_args, filenames):
    ...
    flat["contig_edge_proportion"] = round(
        int(df["partial"].value_counts().loc["True"]) / int(df["partial"].size), 2
    )
    flat["kc_proportion"] = round(
        len(df[df["kc_mibig_acc"] != ""]) / int(df["kc_mibig_acc"].size), 2
    )
    bgc_list = []
...
def main(input_args, filenames):
    ...
    if (
        lib.file_exists(
            filenames.quast_report, "Quast already analysed the assembly!", ""
        )
        is False
    ):
    ...
    if input_args.its is not None:
        ...
        filenames.blast_out = os.path.join(
            itsx_path, f"{input_args.array}_its_blastn.out"
        )
        ...
        if (
            lib.file_exists(its_full, "", "") is False
            and lib.file_exists(its_2, "", "") is False
            and lib.file_exists(its_1, "", "") is False
            and lib.file_exists(its_chi, "", "") is False
        ):
            itsx(input_args, filenames, itsx_path)
        if lib.file_exists(
            its_full,
            "ITSx successfully extracted the full ITS sequence!",
            "ITSx failed to extract the full ITS sequence. Checking for ITS2",
        ):
            filenames.its_fasta = its_full
        elif lib.file_exists(
            its_2,
            "ITSx successfully extracted the ITS2 sequence!",
            "ITSx failed to extract the ITS2 sequence. Checking for ITS1",
        ):
            filenames.its_fasta = its_2
        elif lib.file_exists(
            its_1,
            "ITSx successfully extracted the ITS1 sequence!",
            "ITSx failed to extract the ITS1 sequence. Checking for a chimeric ITS sequence",
        ):
            filenames.its_fasta = its_1
        elif lib.file_exists(
            its_chi,
            "ITSx successfully extracted a chimeric ITS sequence!",
            "ITSx failed... check the logs and your inputs",
        ):
            filenames.its_fasta = its_chi
        else:
            lib.print_h(
                "Skipping taxonomy lookup of isolate as there is no extracted ITS sequence!"
            )
        ...
        try:
            if lib.file_exists(filenames.its_fasta, "", "") is True:
                ...
                if (
                    lib.file_exists(
                        filenames.blast_out,
                        f"BLASTn successfully searched {filenames.its_fasta} against the ITS_RefSeq database!",
                        f"BLASTn failed to search {filenames.its_fasta}... check the logs and your inputs",
                    )
                    is True
                ):
                    its_df = parse_blast(filenames.blast_out, input_args.array)
                    lib.print_n(its_df.head(1))
    ...
    if input_args.antismash is True:
        ...
        filenames.antismash_json = os.path.join(
            antismash_path, f"{input_args.array}.json"
        )
        filenames.antismash_svg = os.path.join(
            results_path, f"{input_args.array}_bgctype_vs_bgclength.svg"
        )
        filenames.bgc_results = os.path.join(
            results_path, f"{input_args.array}_bgc.csv"
        )
        try:
            ...
            if (
                lib.file_exists(
                    filenames.antismash_json,
                    f"antiSMASH already analysed!",
                    f"Need to run antiSMASH",
                )
                is False
            ):
                ...
                if input_args.funannotate is True:
                    ...
                    if (
                        lib.file_exists(
                            filenames.funannotate_func_gbk,
                            f"Using {filenames.funannotate_func_gbk} as input for antiSMASH",
                            f"Using {filenames.assembly_fasta} as input for antiSMASH",
                        )
                        == True
                    ):
                    ...
                    # else will use the prediction GBK file
                        ...
                        lib.file_exists(
                            filenames.funannotate_gbk,
                            f"Using {filenames.funannotate_gbk} as input for antiSMASH",
                            f"Using {filenames.assembly_fasta} as input for antiSMASH",
                        )
                        == True
                    ):
                    ...
                    # else will use the sorted assembly with nicer contig names if Funannotate has been run
                        ...
                        lib.file_exists(
                            filenames.funannotate_sort_fasta,
                            f"Using {filenames.funannotate_sort_fasta} as input for antiSMASH",
                            f"Using {filenames.assembly_fasta} as input for antiSMASH",
                        )
                        == True
                    ):
        ...
        try:
            ...
            lib.print_n("plotting the plots")
            plot_strip(bgc_df, input_args, results_path)
            plot_violin(bgc_df, input_args, results_path)
        except TypeError:
            lib.print_e("No BGCs identified. Skipping BGC plots...")
        except KeyError:
            pass
    else:
        lib.print_n(
            "Skipping BGC search with antiSMASH. Use '--antismash' as a script argument if you would like to do this."
        )
    ...
    filenames.results_csv = os.path.join(
        results_path, f"{input_args.array}_results.csv"
    )
    final_df.to_csv(filenames.results_csv, index=False)
    lib.file_exists_exit(
        filenames.results_csv,
        "Final results report successfully generated",
        "Final results report was not generated",
    )
    lib.print_h(
        f"post analysis module completed in {datetime.datetime.now() - post_start_time}"
    )
