...
#
# Testing on a set of paired Illumina MiSeq reads of ~220 Mb each will take about X hour with 8 cpus and 40 GB memory,
...
final_genome="$TWD"/data/assembly/${SLURM_ARRAY_TASK_ID}/scaffolds.fasta
...
#### end of config file ####
    ...
    final_genome="$TWD"/data/assembly/${SLURM_ARRAY_TASK_ID}/masurca/CA/final.genome.scf.fasta
    if [ -f "$final_genome" ]; then
...
--threads "$cpus" \
--use-names \
--report "$TWD"/output/classified/${SLURM_ARRAY_TASK_ID}_kraken2_contigs_report.txt \
"$final_genome"
...
"$TWD"/data/assembly/${SLURM_ARRAY_TASK_ID}/scaffolds.fasta

antismash_report="$TWD"/output/antismash/${SLURM_ARRAY_TASK_ID}/scaffolds.fasta.gbk
...
if [ "$space_saver" = "on" ]; then
    ...
    rm "$TWD"/data/trimmed/"${SLURM_ARRAY_TASK_ID}_forward_1U.fq.gz"
    rm "$TWD"/data/trimmed/"${SLURM_ARRAY_TASK_ID}_reverse_2U.fq.gz" 
